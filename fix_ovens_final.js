  ...
  while (retries > 0) {
    try {
      ...
      if (e.status === 429 || e.status === 503 || e.message.includes('Quota')) {
        console.log(`Rate limited / Quota (${e.status}), waiting 10s...`);
...
async function run() {
  ...
  // Find all placeholder ovens
  const regex = /\{\s*"id":\s*"([^"]+)",\s*"title":\s*"فرن بلت ان",\s*"titleEn":\s*"Built-in Oven",\s*"category":\s*"افران بلت ان Built-in غاز وكهرباء",\s*"categoryEn":\s*"Built-in Gas & Electric Ovens",\s*"image":\s*"(?:\/public)?\/images\/ovens\/([^"]+)",\s*"description":\s*"فرن بلت ان بمواصفات عالية",\s*"descriptionEn":\s*"High quality built-in oven",\s*"size":\s*"90 سم",\s*"price":\s*""\s*\}/g;
  ...
  for (const m of matches) {
    ...
    if (res) {
      const replacement = [
        '{',
        `    "id": "${m.id}",`,
        `    "title": "${(res.title || 'فرن بلت ان').replace(/"/g, '\\"')}",`,
        `    "titleEn": "${(res.titleEn || 'Built-in Oven').replace(/"/g, '\\"')}",`,
